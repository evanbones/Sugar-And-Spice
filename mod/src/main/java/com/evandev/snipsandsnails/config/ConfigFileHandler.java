...
public class ConfigFileHandler {
    ...
    public static URI getFallbackConfigUri() {
        try {
            return SnipsAndSnails.CLASSLOADER.getResource(FALLBACK_CONFIG_RESOURCE_PATH).toURI();
        } catch (URISyntaxException e) {
            SnipsAndSnails.LOGGER.error("Class loader returned an invalid URI! This should never happen:", e);
            throw new RuntimeException(e);
        }
    }

    public static InputStream getFallbackConfigBytes() {
        return SnipsAndSnails.CLASSLOADER.getResourceAsStream(FALLBACK_CONFIG_RESOURCE_PATH);
    }
